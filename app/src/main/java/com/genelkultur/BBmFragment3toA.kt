...
class BBmFragment3toA : Fragment() {
    ...
    fun getDataGuncelBilgi(bilgi: String) {
        //konu=Guncel
        val ref_t =
            Firebase.database.getReference("BunlariBiliyormusunuz/Guncel/Guncel")//firebase den yolumu belirledim.
        ref_t.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val rnd = (0..snapshot.childrenCount - 1).random()
                    .toInt()//0 dan bilgi sayisina kadar random üret
                val ss = snapshot.children.toList()[rnd] //listele
                val data = ss.child("data").value.toString() //seç
